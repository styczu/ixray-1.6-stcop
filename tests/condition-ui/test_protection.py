...
'''
cpp += 'namespace ConditionUi {\n' + function('src/xrGame/ui/UIConditionFormat.h', '    inline void FormatProtectionPercent') + '}\n'
cpp += r'''
...
struct State {float fill=0,arrow=0;std::string text;bool set_progress(float f){fill=f;return true;}void set_arrow(float f){arrow=f;}void set_text(float){}void set_text_str(const char*s){text=s;}};
struct ui_actor_state_wnd {State*m_state[7];void update_round_states(EStateType,float,float);};
...
int main(){
 ...
 for(int t=0;t<5;++t){auto type=ALife::EHitType(t);
  for(float maximum:{.03f,.2f,.8f})for(float condition:{0.f,.01f,.5f,1.f})for(float resistance:{-.04f,0.f,.02f,.04f,1.2f}){
   ...
   eq(ratios[t](),total/maximum,"XML delegate matches panel");
   ...
   eq(outfitRows[t].m_progress.cur,std::clamp(expectedOutfit/maximum,0.f,1.f)*100,"outfit comparison current");
   eq(outfitRows[t].m_progress.comp,std::clamp(.8f*.25f*.1f/maximum,0.f,1.f)*100,"outfit comparison slot");
   string64 text;ConditionUi::FormatProtectionPercent(text,expectedOutfit/maximum);textEq(outfitRows[t].value.text,text);
   ...
   eq(outfitRows[t].m_progress.cur,std::clamp(expectedHelmet/maximum,0.f,1.f)*100,"helmet current");
   eq(outfitRows[t].m_progress.comp,std::clamp(.6f*.1f*.1f/maximum,0.f,1.f)*100,"helmet comparison");
   panel.update_round_states(statesForHit[t],total,maximum);
   eq(states[statesForHit[t]].fill,std::clamp(total/maximum,0.f,1.f),"only graphical fill clamped");
   ConditionUi::FormatProtectionPercent(text,total/maximum);textEq(states[statesForHit[t]].text,text);
  }
 }
 ...
 afUI.SetInfo(af1);
 for(int t=0;t<5;++t){string64 text;ConditionUi::FormatProtectionPercent(text,.02f/a.conditions().GetZoneMaxPower(ALife::EHitType(t)));textEq(afRows[afRowsForHit[t]].val.text,text);}
 textEq(afRows[1].val.text,"+10%");textEq(afRows[4].val.text,"+2,5%");
 ...
 string64 text;ConditionUi::FormatProtectionPercent(text,1.23456f);textEq(text,"+123,46%");
 ConditionUi::FormatProtectionPercent(text,-.0125f);textEq(text,"-1,25%");
 ConditionUi::FormatProtectionPercent(text,.000001f);textEq(text,"+<0,01%");
 ConditionUi::FormatProtectionPercent(text,0);textEq(text,"0%");
 ...
 ConditionUi::separator='.';ConditionUi::FormatProtectionPercent(text,.125f);textEq(text,"+12.5%");ConditionUi::separator=',';
 ...
 for(int t=0;t<5;++t){auto type=ALife::EHitType(t);for(auto&v:a.cond.m_zone_max_power)v=.2f;
  ...
  assert(p.compute_value({"one"}));textEq(p.text.text,std::string(Protection::Caption(type))+": +2%");
  assert(p.compute_value({"one","two"}));textEq(p.text.text,std::string(Protection::Caption(type))+": +1,5%");
  assert(p.compute_value({"two"}));textEq(p.text.text,std::string(Protection::Caption(type))+": -0,5%");
  settings.data["effect1"].erase(Protection::ConfigKey(type));assert(!p.compute_value({"one"}));
  settings.data["effect1"][Protection::ConfigKey(type)]="0";assert(p.compute_value({"one"}));textEq(p.text.text,std::string(Protection::Caption(type))+": 0%");
...
for p in [root/'gamedata/configs/ui/actor_menu.xml',root/'gamedata/configs/ui/actor_menu_16.xml',args.mod_root/'configs/ui/mod_actor_menu_16_uiparams.xml']:
    ...
    if p.name.startswith('mod_'):
        for kind in widgets.values():
            assert len(tree.findall('.//auto_static[@expression="fltActor'+kind+'ProtectionRatio * 100.0"]'))==1
...
for suffix in ['', '_16']:
    for p in [root/f'gamedata/configs/ui/af_params{suffix}.xml',args.mod_root/f'configs/ui/mod_af_params{suffix}_uiparams.xml']:
        ...
        for kind in ['burn','shock','chemical_burn','radiation','telepatic']:
            ...
            assert value.get('magnitude')=='100' and value.get('unit_str')=='st_proc'
