...
PATCH = HERE / '0001-fix-inventory-drop-preview.patch'
...
def main():
    ...
    # The preview builds on the drop-cell fix. The two patches touch different files,
    # so git am would apply this one on its own and leave the drop bug in place.
    dependency = HERE.parent / 'inventory-drop-cell' / '0001-fix-inventory-drop-cell.patch'
    if not dependency.is_file():
        print('Brakuje pakietu zaleznosci obok tego katalogu: ' + str(dependency), file=sys.stderr)
        return 2
    if git('apply', '--reverse', '--check', str(dependency)).returncode:
        print('Najpierw naloz pakiet inventory-drop-cell; ten go rozszerza.', file=sys.stderr)
        return 2
