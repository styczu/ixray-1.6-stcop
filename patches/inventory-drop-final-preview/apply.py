...
PATCH = HERE / '0001-fix-inventory-drop-final-preview.patch'
...
def main():
    ...
    # Check the patch itself first. It must remain idempotent even though this patch
    # rewrites lines introduced by inventory-cell-grid and can obscure its checks.
    if git('apply', '--reverse', '--check', str(PATCH)).returncode == 0:
        print('Ta poprawka jest już obecna. Nie ma nic do nakładania.')
        return 0

    # inventory-cell-grid introduced CellOffsetUI and the effective cell geometry
    # used by this preview. A source marker is stable when this patch later rewrites
    # the same drag/drop files, unlike reverse-applying the dependency patch.
    dependency_dir = HERE.parent / 'inventory-cell-grid'
    marker_path = args.repo / 'src/xrGame/ui/UIDragDropListEx.h'
    if not dependency_dir.is_dir():
        print('Brakuje pakietu zaleznosci obok tego katalogu: ' + str(dependency_dir), file=sys.stderr)
        return 2
    if not marker_path.is_file() or 'CellOffsetUI' not in marker_path.read_text(errors='replace'):
        print('Najpierw naloz pakiet inventory-cell-grid; ten go rozszerza.', file=sys.stderr)
        return 2
